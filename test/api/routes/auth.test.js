...
describe('/api/v1/auth/login', () => {
  ...
  test('login with correct email and password succeeds', async () => {
    ...
    expect(UserService.authenticateWithPassword).toHaveBeenCalledWith('test@example.com', 'secret');
...
describe('/api/v1/auth/register', () => {
  test('new user can register', async () => {
    UserService.createUser = jest.fn().mockReturnValue({ email: 'test@example.com' });
    ...
    expect(UserService.createUser).toHaveBeenCalledWith({ email: 'test@example.com', password: 'secret' });
  });
});
